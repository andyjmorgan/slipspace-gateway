...
It binds two listeners. Gateways export gen_ai OTLP spans and `slipspace.*` meters over **OTLP gRPC (`:8687`)**; the operator console, query API, and the HMAC-trusted Record webhook (`POST /api/v1/ingest/record`) live on **HTTP (`:8686`)**. The query API is behind Basic auth; the console SPA shell is served unauthenticated and drives its own login form, and the Record webhook authenticates itself with its HMAC signature rather than Basic auth. The fleet-wide console retains full history — the dashboard reads continuous aggregates over the meters, the message inspector lazily joins the verbatim Record blob per request.
