...
|---|---|---|---|
| `Provider` | `string` | `changeProvider` | **Yes (wired).** Read at the final handler ([`handler.go`](../cmd/gateway/handler.go)) and passed to `selection.ResolveTarget` on post-rule state, which re-resolves the upstream endpoint on the post-rule provider. Only superseded when a resilience-policy **target** supplies its own provider action, which the orchestrator applies per attempt — see [`changeProvider`](#changeprovider). |
...
> **v2 routing note (load-bearing).** Under v2 the *destination* — provider, upstream URL, and credential header — is resolved from the configuration's **bindings** by `selectionMiddleware` *before* rules run, and finalised by `buildDestination` in [`cmd/gateway/destination.go`](../cmd/gateway/destination.go) from the selected `selection.Target`. There is no provider/endpoint lookup table; routing stays in bindings. Two state-mutating actions still parse, validate, and round-trip — they are authorable and registered in [`contracts/rules/action.go`](../contracts/rules/action.go) — but the v2 data plane does not consult the state fields they write, so they are **inert**: `changeUrl` is written-but-never-read; `useResiliencePolicy` is ignored in favour of the binding-derived policy on context (and the data plane wires the name lookup to `nil`). `changeProvider`, by contrast, **is** routing-affecting: the final handler reads the post-rule `state.Provider` and calls `selection.ResolveTarget` on it, re-resolving the upstream endpoint on the new provider (invariant #7); a binding-derived provider switch only supersedes the rule's write when a resilience-policy **target** carries its own provider action, applied per attempt by the orchestrator. The remaining actions (`changeApiKey`, `setHeader`, `appendQueryString`, `changeModelName`, `addTag`, the body-rewrite trio, and the two terminating actions) are fully wired — `changeApiKey`'s `state.UpstreamCredentialOverride` is now honoured at the single credential mint site (`resolveCredentialHeaders`, destination.go line 169), and `changeModelName` survives as the orchestrator's internal alias-rewrite primitive, so a binding/group `alias` is the last writer of the body model. Each affected section below carries the specifics. The remaining inert writes are tracked as code follow-ups; the route to a different URL/policy in v2 is a binding edit, not a rule, while a different provider can be reached by either a `changeProvider` rule or a binding (and the upstream credential can now be overridden by `changeApiKey`).
...
> **v2 status — routing-affecting (invariant #7).** The final handler ([`cmd/gateway/handler.go`](../cmd/gateway/handler.go)) reads the post-rule `state.Provider` and passes it to `selection.ResolveTarget` on that post-rule state, re-resolving the upstream endpoint on the **new** provider — this is the model-keyed redirect pattern (e.g. `claude-*` redirected to Anthropic), proven by [`test/e2e/providers/changeprovider_redirect_test.go`](../test/e2e/providers/changeprovider_redirect_test.go). Provider selection also lives in the binding/selection layer: `selectionMiddleware` ([`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go)) walks the configuration's bindings to pick the provider (or resilience group) *before* rules run. The rule's write is only superseded when a resilience-policy **target** supplies its own provider action: the orchestrator's `buildAttemptState` ([`internal/middleware/resilience/middleware.go`](../internal/middleware/resilience/middleware.go) line 683) clones the rules baseline and applies the per-target actions over it. Absent such a target override, the rule-authored `changeProvider` is the value used for routing. It parses, validates, and round-trips (registered in [`contracts/rules/action.go`](../contracts/rules/action.go)). Authoring a binding is an alternative way to route a model to a different provider — see [`docs/providers.md`](providers.md).
...
- `state.Provider` = trimmed `NewProvider` (see `applyChangeProvider` in [`internal/middleware/rules/actions.go`](../internal/middleware/rules/actions.go) line 91). In the v2 data plane the final handler reads this post-rule value and re-resolves the upstream endpoint on the new provider, as described above; it is only superseded when a resilience-policy target supplies its own provider action.
...
The model-keyed redirect pattern (a `claude-*` model posted to an OpenAI-compat path landing on Anthropic) can be expressed either as a rule-authored `changeProvider` — the final handler re-resolves the endpoint on the new provider, proven by [`test/e2e/providers/changeprovider_redirect_test.go`](../test/e2e/providers/changeprovider_redirect_test.go) — or as a binding from the OpenAI `chat` protocol to the `anthropic` provider. The orchestrator's internal `changeProvider`/`changeModelName` pair (`providerSwitchActions` in destination.go line 71) is the same primitive the binding/selection layer drives per attempt.

> **Invariant #7 (load-bearing).** [`CLAUDE.md`](../CLAUDE.md) invariant #7 states "`changeProvider` re-resolves the endpoint on the new provider … reads `state.Provider` post-rule and looks up the endpoint on that provider." This holds for a **rule-authored** `changeProvider` in v2 — the only nuance is *where*: it is the *final handler* ([`cmd/gateway/handler.go`](../cmd/gateway/handler.go)), not `buildDestination`, that reads the post-rule `state.Provider` and re-resolves via `selection.ResolveTarget(state.Protocol, provider, …)` on that post-rule state — `Protocol` is the first argument, the post-rule provider the second. The rule's write is superseded only when a resilience-policy target carries its own provider action, applied by the orchestrator per attempt.

### Gotchas

- `changeProvider` redirects a request by re-resolving the endpoint on the new provider; a binding is an alternative way to route. A rule's write is superseded only when a resilience-policy target supplies its own provider action.
...
- **The action is inert in the v2 data plane** (see the v2-status note at the top of this section). Pairing it with `changeProvider` for "redirect AND wrap in resilience" still won't attach resilience — `useResiliencePolicy` is the no-op here (`changeProvider` does redirect). Attach a resilience group by pointing the binding at `group:` instead.
...
- [`docs/providers.md`](providers.md) — provider registry, per-protocol `auth_header` / `auth_format` conventions, OpenAI-compat surfaces, and the v2 bindings that resolve the destination (an alternative route to a different provider, now that `changeUrl` / `useResiliencePolicy` are inert in the data plane; `changeProvider` and `changeApiKey` are wired — the former re-resolves the endpoint on the new provider, the latter can override the upstream credential).
