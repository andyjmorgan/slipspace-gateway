...
|---|---|---|---|
| `Provider` | `string` | `changeProvider` | **No (inert for routing).** `selectionMiddleware` ([`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go) lines 205-244) synthesises a `ResilienceConfig` for *every* generative request, and every target it builds carries `selection.ProviderSwitchActions` ([`internal/selection/resilience.go`](../internal/selection/resilience.go)), which always includes a `ChangeProviderAction`. The orchestrator's `buildAttemptState` ([`internal/middleware/resilience/middleware.go`](../internal/middleware/resilience/middleware.go) line 702) clones the post-rule baseline and re-applies those per-target actions, so the binding-selected provider always wins. The final handler then reads that provider — never a rule's write. On the passthrough path the destination comes from `pm.Provider` ([`cmd/gateway/handler.go`](../cmd/gateway/handler.go) lines 83-102), which rules cannot reach either — see [`changeProvider`](#changeprovider). |
...
| `OutgoingHeaders` | `http.Header` | `setHeader` | Yes — layered onto the destination by `applyStateOverlays` ([`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go) lines 321-326). |
| `QueryAdditions` | `[]QueryAddition` | `appendQueryString` | Yes — layered onto `dest.UpstreamURL` by `applyStateOverlays` (pipeline.go lines 314-320). |
...
> **v2 status — currently inert.** `applyChangeUrl` ([`internal/middleware/rules/actions.go`](../internal/middleware/rules/actions.go) line 164) parses `newUrl` and stores it on `state.UpstreamURL`, but **nothing in the v2 data plane reads that field.** `buildDestination` builds `dest.UpstreamURL` from the resolved `selection.Target` ([`cmd/gateway/destination.go`](../cmd/gateway/destination.go) lines 126-148, where `upstream` is derived from `target.Path`/`target.Query` and returned as `UpstreamURL`), and the only post-rule overlay step — `applyStateOverlays` ([`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go) lines 313-327) — touches only `QueryAdditions` and `OutgoingHeaders`. `state.UpstreamURL` is written, cloned, and never consulted. The action still parses, validates, and round-trips. Tracked as a code follow-up. To pin a region or override the host in v2, set the provider's base URL in `providers.yaml`.
...
Credential selection is the precedence `switch` in `resolveCredentialHeaders` ([`cmd/gateway/destination.go`](../cmd/gateway/destination.go): func decl at line 172, override-precedence switch at ~line 180), into which `buildDestination` threads the post-rule `state.UpstreamCredentialOverride`. The `changeApiKey` override is checked **before** the auth `Mode`, so a rule wins the last word on the wire. When a credential is minted, the `(header, value)` is formatted by `credentialHeaderFor` (cmd/gateway/destination.go:238): the target's per-protocol auth convention (`target.Auth.Header` / `target.Auth.Format`) when present, falling back to the per-provider-name default in `auth.UpstreamCredentialHeader`. This is the single mint site that honours [`CLAUDE.md`](../CLAUDE.md) invariant 6.
...
The same `resolveCredentialHeaders` mint site is reused for passthrough-family requests in `buildPassthroughDestination` ([`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go) line 253, calling `resolveCredentialHeaders` at line 300), so a `changeApiKey` override applies there too.
...
> 3. The reported `ev.PolicyRef` comes from the `AttemptBuffer` the orchestrator constructed with the *binding-derived* policy name (`abuf.PolicyRef()`, [`cmd/gateway/reporter.go`](../cmd/gateway/reporter.go) line 451), **not** from `state.PolicyRef`. So a rule-authored value does not even reach the connector record or the live feed.
>
...
- `state.PolicyRef` = `strings.TrimSpace(PolicyName)` (`applyUseResiliencePolicy` in [`internal/middleware/rules/actions.go`](../internal/middleware/rules/actions.go) line 374).
...
These three action types are the only actions whose wire contracts live **outside** `contracts/rules/action.go`: `RewriteFieldAction`, `RemoveFieldAction`, and `AppendFieldAction`, along with the target grammar (`ParseTarget` / `ParseReadTarget`), `RewriteValue`, and the `request.body` / `response.body` scope constants, are defined in [`contracts/rules/rewrite.go`](../contracts/rules/rewrite.go) — the three action types at contracts/rules/rewrite.go:228-314, `ParseTarget` at line 55 (strict, identifier segments only) and `ParseReadTarget` at line 75 (permissive, gjson read path), `RewriteValue` at lines 124-223, and the scope constants at lines 25-29. They are registered in the same `actionRegistry` in `action.go` (lines 460-462), so polymorphic dispatch is unchanged.
...
- `gateway.rewrite.applied.total{action_type}` — mutations that applied successfully (the op ran and was not dropped; a `removeField` against an absent path or a `rewriteField` writing an identical value still counts as applied).
...
- **StatusCode outside `[100, 599]` falls back to `500`**. `applyReturnStatusCode` ([`internal/middleware/rules/actions.go`](../internal/middleware/rules/actions.go) lines 388-400) coerces the value inline, which guarantees a misconfigured rule produces a recognisably-bad response rather than a `panic` in `net/http.WriteHeader`.
...
- Do not use `returnStatusCode` or `llmImpersonation` inside a resilience target's `actions` list. The validator does not reject them, but `buildAttemptState` ([`internal/middleware/resilience/middleware.go:702-718`](../internal/middleware/resilience/middleware.go)) discards the `Outcome` returned by `ApplyAction` — the terminating response is silently dropped and the attempt proceeds upstream.
...
`UnknownAction` is the catch-all fallback. When the loader sees a `type:` value the registry doesn't recognise, it constructs an `UnknownAction{Type: <disc>}` and routes all other fields into `DynamicProperties.Extra`. The action round-trips on marshal — every unknown field's value comes back out verbatim (`Extra` holds raw JSON), though `models.MarshalDynamic` re-emits keys in sorted order, so the round-trip is byte-equivalent modulo key order.
