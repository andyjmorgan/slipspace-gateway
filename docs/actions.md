...
> 3. The reported `ev.PolicyRef` comes from the `AttemptBuffer` the orchestrator constructed with the *binding-derived* policy name (`abuf.PolicyRef()`, [`cmd/gateway/reporter.go`](../cmd/gateway/reporter.go) line 448), **not** from `state.PolicyRef`. So a rule-authored value does not even reach the connector record or the live feed.
>
