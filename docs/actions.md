...
- **StatusCode outside `[100, 599]` falls back to `500`**. `applyReturnStatusCode` ([`internal/middleware/rules/actions.go`](../internal/middleware/rules/actions.go) lines 385-398) coerces the value inline, which guarantees a misconfigured rule produces a recognisably-bad response rather than a `panic` in `net/http.WriteHeader`.
