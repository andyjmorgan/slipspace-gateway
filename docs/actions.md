...
- Do not use `returnStatusCode` or `llmImpersonation` inside a resilience target's `actions` list. The validator does not reject them, but `buildAttemptState` ([`internal/middleware/resilience/middleware.go:693-707`](../internal/middleware/resilience/middleware.go)) discards the `Outcome` returned by `ApplyAction` — the terminating response is silently dropped and the attempt proceeds upstream.
