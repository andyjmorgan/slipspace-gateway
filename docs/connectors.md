...
- `date=YYYY-MM-DD` / `hour=HH` — UTC, derived from the segment's record time range: the connector uses the segment's earliest record timestamp (`TsMinNs`, the earliest `Record.TsNs` in the segment) and falls back to its own upload clock only when that is zero — an all-empty range ([`internal/connector/s3/connector.go`](../internal/connector/s3/connector.go) `objectKey`). The partition therefore reflects when the records were captured, not when the segment uploaded.
