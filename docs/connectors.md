...
| `rotation` | object | no | Per-connector override of the spool's default rotation policy. Both sub-fields default when unset. See [Rotation knobs](#rotation-knobs). Spool-backed types only — rejected on `webhook`. |
| `upload_timeout_seconds` | int | no | Per-attempt deadline on `Connector.Upload` for spool-backed connectors. Unset (`0`) applies `DefaultUploadTimeoutSeconds` (60 s); negative aborts the load. Each retry attempt gets its own deadline, so this bounds one wedged PUT, not the whole retry schedule. Rejected on `webhook` (use `timeout_ms`). See [spool.md → Upload, retry, and deadletter](spool.md#upload-retry-and-deadletter). |
...
Either trigger alone seals; whichever fires first rotates. `rotation:` only affects spool-backed connectors (`s3`, `azure_blob`) — it governs how long records batch on disk before a segment uploads. A `rotation:` block on a webhook connector is a config-load error (each record is POSTed immediately, never batched into a segment, so the knob is meaningless there); tune `timeout_ms` and the binding's `max_body_bytes` instead.
...
- `date=YYYY-MM-DD` / `hour=HH` — UTC, derived from the segment's earliest record timestamp (`SealedSegment.TsMinNs`, the smallest `Record.TsNs` in the segment — [`internal/connector/s3/connector.go`](../internal/connector/s3/connector.go) `objectKey`). The uploader reads that value from the stats sidecar the spool writes when the segment seals ([spool.md → Segment stats sidecar](spool.md#segment-stats-sidecar)), so the partition reflects when the records were captured, not when the segment uploaded, and is stable across retries and re-deliveries. The connector falls back to its own upload clock (`c.clock().UTC()`) only when `TsMinNs` is zero — a segment sealed by a pre-sidecar binary, or an `active/` leftover that startup recovery sealed without stats.
- `<unix_ns>-<seq>-<delivery_id>` — the **doubled** filename stem. The on-disk segment is named `<unix_ns>-<seq>.ndjson.zst` ([`internal/spool/segment.go`](../internal/spool/segment.go)); the connector strips the extension to recover the stem `<unix_ns>-<seq>`, then appends `-<delivery_id>` before re-adding `.ndjson.zst`. The catch: `<delivery_id>` **is** that same stem — `deliveryIDFromFilename` derives it by stripping `.ndjson.zst` off the filename ([`internal/spool/track.go`](../internal/spool/track.go)) — so the emitted object name repeats it verbatim: `<unix_ns>-<seq>-<unix_ns>-<seq>.ndjson.zst`. `<delivery_id>` is stable across attempts and the partition comes from record time, so a retry — or a re-delivery after crash recovery moves the segment from `uploading/` back to `sealed/` ([`internal/spool/recovery.go`](../internal/spool/recovery.go)) — is an idempotent overwrite of the same key. The only exception is a sidecar-less segment (upload-clock fallback), where a retry landing in a different UTC hour writes a second key. Consumers should still dedupe on `delivery_id` (or `Record.id`) rather than relying on key collision.
...
The same partition fields and the same doubled `<unix_ns>-<seq>-<delivery_id>` filename derivation as s3 ([`internal/connector/azureblob/connector.go`](../internal/connector/azureblob/connector.go) `blobName`) — the `date=` / `hour=` partition comes from the segment's earliest record timestamp (`TsMinNs`, read from the seal-time stats sidecar), falling back to the connector's upload clock only for a sidecar-less segment, and `delivery_id` equals the segment stem, so the stem repeats in the blob name. See [Object key layout](#object-key-layout) above for the full explanation. The same idempotency caveat applies: with record-time partitioning a retry, or a re-delivery after crash recovery returns the segment from `uploading/` to `sealed/` ([`internal/spool/recovery.go`](../internal/spool/recovery.go)), overwrites the same blob; only the upload-clock fallback can write a second key when the retry lands in a different UTC hour. Dedupe on `delivery_id` (or `Record.id`), not on key collision. The upload streams the raw zstd segment via `UploadStream`; no `Content-Type` or blob metadata is set in v1.
...
The `auth:` block, the `rotation:` block, `upload_timeout_seconds`, and cloud-storage fields (`bucket`, `region`, `account`, `container`, `endpoint_url`, `use_path_style`) are all explicitly rejected at config-load ([`contracts/config/connectors_validate.go`](../contracts/config/connectors_validate.go) `validateWebhook`) — a webhook is a real-time pusher rather than a spooled connector, so every spool knob is inert on it and a stray block is treated as the misconfiguration it is rather than silently ignored.
...
| `Enqueue` after `Close` | Dropped with reason `closed`, never a panic: a record can race the live removal or edit of its connector and reach the pusher the sink reconciler has just closed. |
| no pusher for the bound connector | The reporter finds no sink for the name (a live-created connector whose `secret_ref` did not resolve, or a binding that outlived its connector) and bumps `gateway.telemetry.push.dropped.total` with reason `no_sink`. |
