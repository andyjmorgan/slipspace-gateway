...
- `date=YYYY-MM-DD` / `hour=HH` — UTC, taken from the connector's own upload clock (`c.clock().UTC()`). The connector does read the segment's earliest record timestamp (`TsMinNs`, the earliest `Record.TsNs` in the segment) when it is non-zero ([`internal/connector/s3/connector.go`](../internal/connector/s3/connector.go) `objectKey`), but the spool uploader populates only `Path`, `DeliveryID` and `Connector` on the `SealedSegment` ([`internal/spool/track.go`](../internal/spool/track.go), `uploadOne`), so `TsMinNs` is always zero in production and that branch is currently dead code, retained for when the uploader is fixed ([#440](https://github.com/andyjmorgan/slipspace-gateway/issues/440)). The partition therefore reflects when the segment uploaded, not when the records were captured.
- `<unix_ns>-<seq>-<delivery_id>` — the **doubled** filename stem. The on-disk segment is named `<unix_ns>-<seq>.ndjson.zst` ([`internal/spool/segment.go`](../internal/spool/segment.go)); the connector strips the extension to recover the stem `<unix_ns>-<seq>`, then appends `-<delivery_id>` before re-adding `.ndjson.zst`. The catch: `<delivery_id>` **is** that same stem — `deliveryIDFromFilename` derives it by stripping `.ndjson.zst` off the filename ([`internal/spool/track.go`](../internal/spool/track.go)) — so the emitted object name repeats it verbatim: `<unix_ns>-<seq>-<unix_ns>-<seq>.ndjson.zst`. `<delivery_id>` is stable across attempts, so the *filename* never changes — but the `date=` / `hour=` partition is re-derived from the connector's upload clock on every attempt (because `TsMinNs` is always zero, [#440](https://github.com/andyjmorgan/slipspace-gateway/issues/440)). S3's put-object semantics therefore make a retry an idempotent overwrite only when it lands in the same UTC hour; a retry — or a re-delivery after crash recovery moves the segment from `uploading/` back to `sealed/` ([`internal/spool/recovery.go`](../internal/spool/recovery.go)) — writes a *different* key and leaves a duplicate object at the old partition only when it lands in a different UTC hour. Consumers must dedupe on `delivery_id` (or `Record.id`) rather than relying on key collision.
...
The same partition fields and the same doubled `<unix_ns>-<seq>-<delivery_id>` filename derivation as s3 ([`internal/connector/azureblob/connector.go`](../internal/connector/azureblob/connector.go) `blobName`) — including that the `date=` / `hour=` partition always comes from the connector's own upload clock because the uploader leaves `TsMinNs` zero ([#440](https://github.com/andyjmorgan/slipspace-gateway/issues/440)), and that `delivery_id` equals the segment stem, so the stem repeats in the blob name. See [Object key layout](#object-key-layout) above for the full explanation. The same caveat applies: `<delivery_id>` is stable across attempts so the blob *name* never changes, but the `date=` / `hour=` partition is re-derived from the upload clock on every attempt, so Azure Blob Storage's put-blob semantics make a retry an idempotent overwrite only within the same UTC hour — a retry, or a re-delivery after crash recovery returns the segment from `uploading/` to `sealed/` ([`internal/spool/recovery.go`](../internal/spool/recovery.go)), writes a different key and leaves a duplicate blob at the old partition only when it lands in a different UTC hour. Dedupe on `delivery_id` (or `Record.id`), not on key collision. The upload streams the raw zstd segment via `UploadStream`; no `Content-Type` or blob metadata is set in v1.
