...
Provider-reported token usage extracted from the upstream response body. Input and output usage ride the `gen_ai.client.token.usage` histogram (keyed by `gen_ai.token.type=input|output`, which replaced the former input/output counters); cache tokens ride two `sluice.*` counters. All share the same base **request labelset** (`requestDimensionAttrs` — `gen_ai.provider.name`, `gen_ai.request.model`, `gen_ai.operation.name`, `sluice.protocol`, `sluice.configuration`, plus `gen_ai.response.model` when the response carried a model and `server.address`/`server.port` when the upstream host is known), so token rate and traffic rate join cleanly in Grafana. The two `gen_ai.client.token.usage` rows add `gen_ai.token.type=input|output` on top of that base; the cache counters carry the base alone and **no HTTP status** — token instruments are status-free. They are recorded **only for non-zero buckets** — a request that doesn't include `usage` produces no observation rather than a zero-valued one. There are no `gateway.tokens.input.total` / `gateway.tokens.output.total` instruments.
