...
The console exposes both read-only inspection surfaces (dashboard, live messages, policies, bindings, export) and a write API spanning most of the policy YAML: full CRUD for **Configurations, Providers, Groups, Connectors, Rules, and API Keys**. Those five non-key resources expose `GET`/`POST` on the collection and `GET`/`PUT`/`DELETE` on `/{name}`; `PATCH` exists only on `/api/v1/config/api-keys/{id}` (the `enabled` toggle). Bindings are exposed read-only (they are edited as part of a Configuration). Every write clones the live config snapshot, validates the clone, persists the YAML atomically, and publishes via `config.Store.Replace` — no pod restart, and in-flight requests are unaffected. Connector writes also reach the runtime: a store subscriber (`cmd/gateway/sinks.go`) registers the spool track or webhook pusher for a created connector, stops it for a deleted one, and swaps it for an edited one, so records flow to the new destination from the next request ([deployment.md → Restart-to-apply](deployment.md#restart-to-apply-non-admin-paths-only)). The write API requires `SLIPSPACE_CONFIG_DIR` to be writable; see [Configuration mount → Read-write config dir](deployment.md#read-write-config-dir-admin-write-api) for the production pattern.
