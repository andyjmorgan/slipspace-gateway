...
Every Required/CR/Recommended attribute: operation/provider/request.model, response id/model/finish_reasons, request params (temperature, top_p, top_k, max_tokens, frequency/presence penalty, stop_sequences, seed, choice.count, stream), output.type, conversation.id, agent.id (resolved from `X-Slipspace-Agent-Id` only, plus operator-configured `SLIPSPACE_AGENT_ID_HEADERS` entries; emitted when present — `X-Claude-Code-Agent-Id` no longer feeds agent.id, it feeds the conversation/thread axis (`DefaultThreadIDHeaders`) since its values are opaque per-invocation instance ids, not named agents), enduser.id (resolved from `X-Slipspace-User-Id`; emitted when present — the GenAI semconv has no user attribute, so the end user rides the general `enduser` namespace), usage (input/output/cache_creation/cache_read/reasoning), response.time_to_first_chunk, server.address/port, `http.response.status_code` (set on every request span), error.type. Plus `openai.*` (request/response service_tier, system_fingerprint, api.type). The `openai.api.type` value comes from `cmd/gateway/handler.go::openAIAPIType`, which maps **only** the `chat` protocol to the well-known surface name `chat_completions`; every other protocol (`responses`, `embeddings`, …) passes through unchanged — there is no `responses`→`chat_completions` rewrite.

**Resilience attempt child spans.** A request the orchestrator drove across multiple upstream attempts becomes a parent request span with one CLIENT child span per attempt (name `{operation.name} {request.model} [{target}]`), backdated from each attempt's `StartedAt` + `DurationMs`. Each child carries `slipspace.resilience.target` and `slipspace.resilience.outcome` (`success` / `failure_status` / `transport_error` / `cb_blocked`), plus `http.response.status_code` and `error.type` on a failed attempt. These are SlipSpace-namespaced extras (the spec has no per-attempt concept); the parent span stays spec-conformant.

### Events (OTel logs pipeline)
- `gen_ai.client.operation.exception` — on every failure (status ≥ 400 or transport error).
- `gen_ai.client.inference.operation.details` — bounded prompt/response content as **structured** log values per the message JSON schema: `gen_ai.input.messages` / `gen_ai.output.messages` as `[{role, parts}]`, `gen_ai.system_instructions` as a bare parts array, `gen_ai.tool.definitions` normalised to `{type,name,description,parameters}`. Parts carry the well-known types `text` / `tool_call` / `tool_call_response` / `reasoning`; tool responses use the OTel 1.37 `response` field (not the legacy internal `result` name). Anthropic `thinking` + `redacted_thinking` map to reasoning, with redacted thinking emitting an empty-content part; media blocks pass through by type. It carries `slipspace.protocol` and `slipspace.configuration` on top of the shared operation-detail attributes (`emitOperationDetails`, `cmd/gateway/reporter.go:1530-1531`, emitting `AttrSlipSpaceProtocol` at :1530 and `AttrSlipSpaceConfiguration` at :1531); the span sets the same two facts independently via `appendSlipSpaceFactAttrs` (`cmd/gateway/reporter.go:1396`, emitting `AttrSlipSpaceConfiguration` at :1398 and `AttrSlipSpaceProtocol` at :1401), which invariant #4 depends on — the TimescaleDB CAGGs read them off the span as metric labels. Opt-in via `SLIPSPACE_OTEL_CAPTURE_CONTENT` (default off), per-field capped, and credential-redacted (`internal/contentredact`). Emitted within the span context so records carry `trace_id`/`span_id`.
...
- **Dropping `slipspace.requests.total` and the `slipspace.tokens.cache*` counters.** These are spec-legal additive extras under the `slipspace.*` namespace, and they back the admin console's request/cache aggregation (the console reads metrics, not spans/events). The cache *data* also rides the `gen_ai.usage.cache_*` span/event attributes, but those are per-request, not aggregatable in the console — so dropping the counters would regress the console for zero conformance benefit. Kept.
...
- **Cosmetic:** the internal identifiers have since been renamed to match the wire string — `MetricTimeToFirstChunk` / `TimeToFirstChunk` (`internal/observability/meters.go`), emitting `gen_ai.client.operation.time_to_first_chunk`. The rename covers the exported OTel metric identifiers only — `MetricTimeToFirstChunk`, `TimeToFirstChunkBuckets` and the `TimeToFirstChunk` field. First-byte naming still exists off the telemetry channel by design: the Record field `Response.FirstByteNs` (`cmd/gateway/reporter.go:793`), the `ttfb_ms` log key (`:577`) and the `ttfbMs`/`ttfb` locals (`:424`, and the `ttfb` histogram local in `internal/observability/meters.go`). Per CLAUDE.md invariant #4 the Record and log channels are deliberately separate from OTel, so those names are not a semconv conformance gap.
