...
| `connector_bindings` | []ConnectorBinding | no | Attaches one or more top-level connectors with per-binding sampling / filter / size-cap overrides. Empty (or absent) means no records are captured — the reporter skips record emission when a configuration has no bindings (`cmd/gateway/reporter.go:602`); body capture itself still runs, feeding the rules engine and the admin live feed. See [connector-bindings.md](connector-bindings.md). |
