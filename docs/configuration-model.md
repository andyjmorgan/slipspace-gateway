...
| `rules` | Top-level rule library (body/header/query rewrites, tags, short-circuits, and the one wired steering action `changeApiKey`). Bindings carry the default routing as config data; a rule `changeProvider` overrides it for the requests it matches, taking them out of the bound group (GitHub #294). `changeUrl` and `useResiliencePolicy` are **inert** in v2 (they parse but the data plane ignores them). Pure body/header/query transforms are the common case. Configurations reference rules through `rule_names`. |
...
| `timeout_seconds` | int | no | When `> 0`, a whole-attempt wall-clock bound (context deadline covering connect, headers and body) on every attempt under this group; a timed-out attempt fails over like a transport error. `Target.timeout_seconds` overrides it per target. Zero leaves attempts unbounded beyond the header timeout. See [`resilience.md` → Attempt timeout and retry backoff](resilience.md#attempt-timeout-and-retry-backoff). |
| `retry` | *RetryConfig | no | Inter-attempt backoff and attempt budget for the failover / re-roll walk: `enabled`, `max_attempts`, `backoff_type` (`constant`, `linear`, `exponential`), `delay_ms`, `max_delay_ms`, `use_jitter` (`contracts/resilience/types.go`, `RetryConfig`). Omitted or disabled keeps the default (next target immediately, every target may be tried). Validated by `RetryConfig.Validate` at load. |
| `targets` | []Target | yes | The providers this group routes across. Must have at least one; two entries that are the same use of one provider (identical `provider`, `alias`, `path`, `query`) are rejected, while the same provider with a different alias, path or query may be listed more than once (`internal/config/config_validate.go::validateGroups`). |
...
| Field | Type | Required | Notes |
|---|---|---|---|
| `provider` | string | yes | Names a provider in the top-level `providers` block. Unknown provider aborts validation; so does a second entry that repeats another target's provider, alias, path and query exactly. |
...
| `timeout_seconds` | int | no | Whole-attempt wall-clock bound for this target only; overrides `Group.timeout_seconds`. Zero inherits the group value. |

The target's runtime identity — the circuit-breaker key and the `target` metric label — is `Group.TargetNames()` (`contracts/config/model.go`): the plain provider name when the provider appears once in the group, otherwise `provider#alias` or `provider#<1-based position>`.

Validation (`internal/config/config_validate.go::validateGroups`): the group name is an identifier; `mode` is set; the group declares at least one target; every target names a `provider` that exists, and no two targets are the same use of one provider (identical `provider`, `alias`, `path`, `query`). The group is then synthesised into the orchestrator's `ResilienceConfig` (`selection.GroupResilienceConfig`, `internal/selection/resilience.go`) and `contracts/resilience` `ResilienceConfig.Validate()` runs over it — closed-set `mode`, 4xx/5xx `failure_status_codes`, non-negative `response_header_timeout_seconds`, and the full `circuit_breaker` rule set (rate in `[0, 1]`, no negatives, enabled breaker needs a trip arm and `cooldown_seconds > 0`). Rejections wrap `ErrValidation` and the originating `contracts/resilience` sentinel. See [`resilience.md` → Validated at load](resilience.md#validated-at-load). The protocol-preserving check happens at the **binding** level — when a binding references a group, every target in that group must serve the binding's protocol (`validateBindings`, `config_validate.go:285`).
...
| `connector_bindings` | []ConnectorBinding | no | Attaches one or more top-level connectors with per-binding sampling / filter / size-cap overrides. Empty (or absent) means no records are captured — the reporter skips record emission when a configuration has no bindings (`cmd/gateway/reporter.go:608`); body capture itself still runs, feeding the rules engine and the admin live feed. See [connector-bindings.md](connector-bindings.md). |
...
`rules:` is the top-level rule library, a flat list of `RuleContract` entries (`contracts/rules`). In v2 bindings carry the default routing; a rule can override the upstream credential per-request with `changeApiKey` (honoured at the single mint site `resolveCredentialHeaders`) and the upstream provider with `changeProvider` (which takes precedence over the bound group / alias — see [actions.md](actions.md#changeprovider)), but the legacy `changeUrl` and `useResiliencePolicy` actions are **inert** (they parse but the data plane ignores the state they write — see [actions.md](actions.md#changeurl)). A rule can also apply body / header / query rewrites, tags, and short-circuits. Definitions are unique by `name`; configurations reference them through `Configuration.RuleNames`. See [rules.md](rules.md) for the condition/action grammar.
...
`connectors:` may be empty or absent — when there are no connectors, the spool is not constructed (`cmd/gateway/main.go:370-372`) and the reporter emits no records. This is the default for any deployment that does not want persistent capture.
...
| `ErrValidation` (group) | Group name is not an identifier; `mode` missing; group has no `targets`; a target has no `provider`; a target names an unknown provider; two targets are the same use of one provider (identical alias, path and query); or the synthesised `ResilienceConfig` fails `contracts/resilience` validation — the error also wraps that sentinel (`ErrUnknownMode`, `ErrInvalidCircuitBreakerConfig`, `ErrInvalidFailureStatusCode`, `ErrInvalidThreshold`, `ErrInvalidRetryConfig`, `ErrUnknownBackoffType`, `ErrDuplicateTargetName`). |
