...
YAML keys are camelCase by convention (`expectedModelName`, `keyOperator`); the equivalent JSON wire format is snake_case (`expected_model_name`, `key_operator`). Each decoder accepts only its own shape. YAML config is decoded against the camelCase `yaml` tags; the admin JSON API is decoded against the snake_case `json` tags. The two are not interchangeable — a camelCase key in a JSON payload does not bind to the typed field, it is preserved verbatim in `DynamicProperties.Extra` and round-trips back out unchanged (invariant #1), which means a typo'd or wrong-cased key silently has no effect rather than erroring. That preservation is JSON-only: `Extra` is tagged `yaml:"-"` (`models/dynamic.go`) by design — operator-authored YAML carries no provider-side unknown fields — so a mis-cased key in a YAML rule is silently ignored rather than round-tripped. The wire format you see in the admin console's JSON tab will differ from your YAML, by design.
...
One discriminator is exempt from that inertness: the retired `endpoint` type, renamed to `protocol`. `internal/config/config_validate.go` walks every rule condition recursively — including `group` children — and fails config load with `ErrRetiredEndpointCondition` (`config: rule condition "endpoint" was renamed to "protocol"`) if it finds one. A config still carrying `type: endpoint` still decodes to an inert `UnknownCondition` — that is exactly how the validator spots it (`conditionHasRetiredEndpoint` type-switches on `*UnknownCondition` with `Type == "endpoint"`, `internal/config/config_validate.go:183-195`) — but the config then refuses to load, so the breaking rename fails loud instead of silently disabling the rule.
