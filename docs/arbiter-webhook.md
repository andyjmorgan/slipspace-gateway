...
- **Gateway side — the pusher** ([internal/arbiter/pusher/pusher.go](../internal/arbiter/pusher/pusher.go)).
  ...
  OTLP weight.
- **Service side — the ingest handler** ([internal/arbiter/ingest/record.go](../internal/arbiter/ingest/record.go),
  `RecordHandler`). Verifies the HMAC against the registered gateway secret
  ([internal/arbiter/registry/registry.go](../internal/arbiter/registry/registry.go)),
...
Mounted on the Arbiter's HTTP listener (`http_bind`, default `0.0.0.0:8686`)
in [internal/arbiter/server/server.go](../internal/arbiter/server/server.go) at
...
([internal/arbiter/pusher/pusher.go](../internal/arbiter/pusher/pusher.go)).
...
Receiver ([registry.go](../internal/arbiter/registry/registry.go), `Registry.Verify`):
...
See [arbiter-database-schema.md](arbiter-database-schema.md) for the `record`
table shape and [arbiter.md](arbiter.md) for how the console
...
- **Record row** — `ON CONFLICT (correlation_id) DO UPDATE`
  ([internal/arbiter/store/record.go](../internal/arbiter/store/record.go)). A
...
`pusher.New` ([internal/arbiter/pusher/pusher.go](../internal/arbiter/pusher/pusher.go))
...
Arbiter's ingest is an idempotent upsert keyed by `correlation_id`. Other 4xx
...
- [arbiter.md](arbiter.md) — the service that terminates this webhook plus the two OTLP feeds.
- [arbiter-api.md](arbiter-api.md) — the console query API over the stored events.
- [arbiter-database-schema.md](arbiter-database-schema.md) — the `record` blob table and the span-written `request_events` entity it joins to.
