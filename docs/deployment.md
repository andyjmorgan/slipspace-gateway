...
Config edits via the admin write API apply live — `config.Store.Replace` swaps the snapshot atomically and the next request evaluates against the new config. Live write APIs cover rules, providers, groups, configurations, api_keys, and connectors (each clones the snapshot, validates, persists back to the block's source file, then publishes through `config.Store.Replace` — no restart). For connectors the runtime sinks follow the snapshot too: a sink reconciler subscribed to the store registers a spool track (s3 / azure_blob) or builds a webhook pusher for a created connector, tears the sink down for a deleted one, and realises an edit as unregister-then-register under the same name — the spool backlog stays on disk across the swap ([spool.md → Live track registration](spool.md#live-track-registration-and-removal)). If a live-created connector cannot be built (unresolvable `secret_ref`, bad credentials) the write still succeeds and the failure is logged at error level; records bound to it are then counted as `gateway.spool.dropped.total{reason="no_track"}` / `gateway.telemetry.push.dropped.total{reason="no_sink"}` rather than lost silently. **Direct YAML edits on disk** (e.g. updating the source ConfigMap, editing the PVC contents from a sidecar, manual `kubectl cp`) still require a process restart — the in-binary `fsnotify` watcher is a v1.2+ task. To apply a direct edit, roll the Deployment (`kubectl rollout restart deployment/slipspace-gateway`). The `admin` and `telemetry` blocks have no write API; changes to those also require a restart.
