...
For each attempt the orchestrator wraps the response writer in a `BufferingResponseWriter`. It intercepts `WriteHeader` (the commit-or-discard decision point) and stages `Header()` writes in an isolated map so a discarded attempt's headers never reach the client — body bytes are never buffered; `Flush` is a no-op until commit. The status line is the commit-or-discard decision point: a status in the attempt's retry set (or a transport error with no `WriteHeader` at all) is swallowed and `Committed` stays false, so the next attempt can run; any other status passes straight through to the real `http.ResponseWriter` and commits, after which body writes stream through untouched.
...
- **Telemetry request inspector**: when an entry carries any attempts (`attempts.length > 0`), the inspector renders a "Resilience attempts" section holding a per-attempt breakdown grid ([`web/src/telemetry/components/inspector-body.tsx`](../web/src/telemetry/components/inspector-body.tsx)) with four columns — attempt index, target, status code (falling back to the transport error string, then `—`), and outcome — so a single-attempt orchestrated request shows a one-row table, not a hidden one. This is the only SPA surface that renders attempts today: the gateway admin console's live-messages modal (`web/src/pages/messages.tsx`) does not display them — there is no attempts table and no `policy_ref` chip there. `duration_ms` is carried on the wire by both the connector Record ([`contracts/connector/record.go`](../contracts/connector/record.go) `Attempt.duration_ms`) and the admin live-feed `AttemptHit` ([`contracts/admin/messages.go`](../contracts/admin/messages.go) `AttemptHit.duration_ms`, projected from the internal [`internal/observability/livefeed/entry.go`](../internal/observability/livefeed/entry.go) struct), but no SPA surface currently displays it.
...
**"My group never runs."**
Check the binding actually selects it. The connector `Record` carries `policy_ref` per request when a group ran (no SPA surface renders it — see the SPA surfaces note above); check a captured record or the `gateway.resilience.*` metrics' `policy` label. If `policy_ref` is empty, the matched binding pointed at a single `provider:` (single-shot), not a `group:` — or no binding matched at all (the request 404s). Verify the binding's `protocol` matches the inbound path and its `models` patterns match the request model.
