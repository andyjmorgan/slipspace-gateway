...
- **[`internal/proxy/forwarder.go`](../internal/proxy/forwarder.go)** — the forwarder and its `alwaysDropHeaders` (`forwarder.go:189`), the unconditional inbound-strip list (`X-Slipspace-Configuration`/`X-Slipspace-Identity`/`Authorization`/`Origin`/`Referer`/`Cookie`/`Accept-Encoding` — seven entries).
