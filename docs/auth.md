...
| `X-Slipspace-Agent-Id`, `X-Claude-Code-Agent-Id` | correlation middleware (echoes only) | Optional client-supplied agent identifier (the agent/sub-agent that issued the request). `X-Slipspace-Agent-Id` is authoritative; `X-Claude-Code-Agent-Id` is the shipped fallback, plus any `SLIPSPACE_AGENT_ID_HEADERS` extras. The resolved id is echoed on the response under `X-Slipspace-Agent-Id`. See [observability.md → Agent id](observability.md#agent-id). | Forwarded upstream as part of the inbound request. |
