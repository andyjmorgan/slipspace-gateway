...
| `X-Sluice-User-Id` | correlation middleware (echoes only) | Optional client-supplied end-user identifier (the user on whose behalf the request was made). `X-Sluice-User-Id` is authoritative; there is no shipped client default, so only `SLUICE_USER_ID_HEADERS` extras extend the chain. The resolved id is echoed on the response under `X-Sluice-User-Id`. See [observability.md → User id](observability.md#user-id). | Forwarded upstream as part of the inbound request. |
