...
A sibling `backfill_runs (name PK, completed_at)` table (migration 11) tracks the **out-of-band data backfills** the schema migrations deliberately defer — currently `v10_token_columns` ([`backfill.go::BackfillTokenColumns`](../internal/arbiter/store/backfill.go)), `v12_tags_column` ([`backfill.go::BackfillTags`](../internal/arbiter/store/backfill.go)), and `v20_cost_column` ([`backfill.go::BackfillCost`](../internal/arbiter/store/backfill.go)). All three re-project a column migrations added metadata-only — `tokens_in`/`tokens_out` (migration 10), `tags text[]` (migration 12), `cost_usd DOUBLE PRECISION NOT NULL DEFAULT 0` (migration 20) — because the inline `UPDATE` detoasted every row and outlasted the liveness probe. Unlike `schema_migrations`, an absent row here is normal mid-flight: the backfill runs batched in the background after boot and records its name only on completion, so an interrupted run resumes (idempotently) on the next boot.
