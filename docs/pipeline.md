...
| `selectionMiddleware` | [`cmd/gateway/pipeline.go`](../cmd/gateway/pipeline.go) | Walk the configuration's bindings to pick a provider or resilience group; seed `MutableState`. Constructed as `selectionMiddleware(store, agentRouter, errs, next)` — the agent router ([`internal/agentroute.Service`](../internal/agentroute)) is threaded in so agent-aware routing can override the binding-selected target before the resilience config is attached. The only stage that returns 404/405. |
