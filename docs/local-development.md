...
| `dev-with-overlay` | `docker compose -f docker-compose.yaml -f docker-compose.dev.yaml up -d mockllm` + `go run ./cmd/gateway` | same as `dev` | — | Requires `docker-compose.dev.yaml` (copy from `.example`). Service-scoped exactly like `dev` — it brings up only `mockllm` (with the overlay applied), not the whole stack; the gateway still runs natively. |
