...
| `SLUICE_SESSION_ID_HEADERS` | _(empty)_ | string (CSV) | Comma-separated, ordered list of inbound header names treated as session-id aliases. The canonical `X-Sluice-Session-Id` always wins; when it is absent the correlation middleware consults these headers in order and uses the first non-empty value. The resolved session id is echoed back on `X-Sluice-Session-Id` and attached to the per-request logger as `session_id`. Lets clients that emit their own session header (e.g. `X-Agentling-Task-Id`) participate in session correlation without changing their request code. | Whitespace-trimmed per entry, empty entries dropped, operator order preserved. Empty / unset keeps resolution at the canonical header only. | [Observability](#observability--otlp-prometheus-logging) |
...
| `SLUICE_SESSION_ID_HEADERS` | _(empty)_ | string (CSV) | Inbound header names treated as session-id aliases when `X-Sluice-Session-Id` is absent. Resolved value is echoed back and logged as `session_id`. |
