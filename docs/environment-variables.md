...
| `SLUICE_AGENT_ID_HEADERS` | _(empty)_ | string (CSV) | Comma-separated header names appended, in order, to the built-in agent-id fallback chain (`X-Claude-Code-Agent-Id`). The authoritative `X-Sluice-Agent-Id` is always tried first regardless. Lets an operator promote a custom client's agent header with no code change. A header also matched by `SLUICE_REDACT_EXTRA_HEADERS` is skipped during resolution, so a promoted agent id can never bypass redaction. | Whitespace-trimmed per entry, empty entries dropped. Empty / unset uses the built-in chain only. | [Observability](#observability--otlp-prometheus-logging) |
...
| `SLUICE_AGENT_ID_HEADERS` | _(empty)_ | string (CSV) | Extra agent-id fallback headers, appended after the built-in chain (`X-Claude-Code-Agent-Id`). `X-Sluice-Agent-Id` always wins. |
