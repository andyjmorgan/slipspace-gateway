...
| `unexpected_config_file` | **Legacy (v1) code.** Unreachable under the v2 schema — the loader merges every `*.yaml` file (the `.yml` spelling is **not** recognised — `internal/config/config_model.go` filters on `filepath.Ext(name) != ".yaml"`, so a `.yml` file is silently skipped) by top-level key with no fixed-filename allowlist (`internal/config/config_model.go:129-161`). Its sentinel, `config.ErrUnexpectedConfigFile` (`internal/config/errors.go:9-13`), is a retained v1 artifact that `Load` never returns, so the category is never emitted for a v2 bundle. |
| `wrong_file_for_key` | **Legacy (v1) code.** Same as above — v2 allows any key in any file, so its sentinel `config.ErrWrongFileForKey` (`internal/config/errors.go:15-19`) is likewise retained but never returned. Never emitted for a v2 bundle. |
...
| `/control/state` | `GET` | Returns `{"responses": [...], "sessions": {...}, "request_count": N}` — with nothing staged, `{"responses": [], "request_count": 0}`. `responses` is the global pool; `sessions` only contains non-empty per-session pools and is `omitempty` — it appears only once at least one session-scoped response has been staged. |
...
| `/healthz` | `GET` | Returns 200 with body `ok`. The Python wire-compat harness (`test/python/conftest.py`) uses it as the mockllm readiness probe before spawning the gateway; the Go e2e harness (`test/e2e/harness/harness.go`) instead probes `GET /control/state` and requires exactly 200 (its impostor guard needs the control-plane response, not a generic health page). No compose file defines a healthcheck for mockllm — the only compose healthchecks in the repo are the Postgres `pg_isready` probes in `docker-compose.arbiter.yaml` and `deploy/quickstart/compose.arbiter.yaml`. |
