...
| Forwarded upstream | **Stripped.** The current `X-Slipspace-Identity` / `X-Slipspace-Configuration` headers are in the forwarder's unconditional [`alwaysDropHeaders`](../internal/proxy/forwarder.go) list; the retained pre-rebrand aliases `X-Sluice-Identity` / `X-Sluice-Configuration` are stripped a layer earlier, via `AuthResult.DropHeaders` (`passthroughDropHeaders`, [`internal/middleware/auth/resolver.go:241-247`](../internal/middleware/auth/resolver.go)), which seeds all four selectors on every *successful* managed and passthrough resolution (unauthorised and unknown-configuration returns carry no DropHeaders, but those requests are never forwarded upstream). Either way the upstream provider never sees a SlipSpace-internal selector. |
