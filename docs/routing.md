...
| Forwarded upstream | **Stripped.** The current `X-Slipspace-Identity` / `X-Slipspace-Configuration` headers are in the forwarder's unconditional [`alwaysDropHeaders`](../internal/proxy/forwarder.go) list; the retained pre-rebrand aliases `X-Sluice-Identity` / `X-Sluice-Configuration` are stripped a layer earlier, via `AuthResult.DropHeaders` (`passthroughDropHeaders`, [`internal/middleware/auth/resolver.go:243-245`](../internal/middleware/auth/resolver.go)), which seeds all four selectors on every managed *and* passthrough resolution. Either way the upstream provider never sees a SlipSpace-internal selector. |
...
> **Note on config-load errors.** `internal/config/errors.go` still defines some v1-era names (`ErrPathCollision`, `ErrPrefixRequiredEmpty`). These belong to the legacy v1 model and are **not** part of v2 routing — v2 has no path-based route table to collide and no provider prefixes. `cmd/cli/validate.go`'s `classifyConfigErr` still branches on both (tested in `main_test.go`), but nothing in the v2 codebase ever returns either error, so the sentinels and that CLI classification arm are unreachable v1 residue that could be removed together. v2 config validation instead checks binding/provider/group references, protocol coverage, and credential presence. See [docs/configuration-model.md](configuration-model.md) for the validation surface.
