...
- **Basic-auth console + query API** — `GET /api/v1/dashboard/*`, `/api/v1/messages*`, `/api/v1/events*`, `/api/v1/sessions/{id}`, `/api/v1/facets`, `/api/v1/tool-calls*`, `/api/v1/verdict/{id}`, `/api/v1/findings` and `/api/v1/advise/audit*` (`internal/arbiter/server/query.go::registerQueryRoutes`, mounted only when a query store is attached), plus `GET /api/v1/settings`, which `Server.Handler` mounts directly (`server.go:113-116`) and only when `WithAppliedConfig` attached a redacted snapshot — independent of the query store. Every API route is wrapped in `Server.basicAuth` (`server.go:161-170`). The SPA bundle itself is served **unauthenticated** at `GET /` (`server.go:117-119`) — only the API it calls is gated; the SPA drives its own login form. See [arbiter-api.md](arbiter-api.md).
