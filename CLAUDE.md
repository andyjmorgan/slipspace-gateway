...
- **Test surface per model type:** golden round-trips are inline table-driven cases in each `protocols/*/*_test.go` (byte-equivalent modulo key order); `test/fixtures/` holds E2E fixtures, not the per-type golden round-trips; fuzz targets on every `UnmarshalJSON` ("if it parses, it round-trips") — note CI runs them as ordinary seed-corpus subtests under `go test -race`, with no `-fuzz`/`-fuzztime` budget, so a dedicated long-running fuzz job is a gap, not a shipped control; unknown-discriminator + unknown-field tests via `UnknownX` / `DynamicProperties.Extra`; a `TestX_AllExportedFieldsHaveJSONTag` reflection meta-test enforcing `json` tags.
