...
7. **Transport is re-resolved from post-rule state via `selection.ResolveTarget`, never from an in-destination override table.** After rules run, the handler calls `selection.ResolveTarget(state.Protocol, provider, …)` (`cmd/gateway/handler.go`) to resolve a `selection.Target` on the **post-rule** protocol + provider; that Target carries the upstream base URL, protocol path, auth convention, default query, and the configuration's credential. `buildDestination` (`cmd/gateway/destination.go`) consumes the pre-resolved Target as the single credential mint site — it does **not** look up an endpoint map or carry a `changeUrl` override table. Model-keyed redirect is primarily expressed as a binding (models pattern → provider) on the Configuration, not a `changeProvider` rule (`config-dev/policy.yaml`; `docs/providers.md`); the orchestrator drives it per attempt through its internal primitive (`selection.ProviderSwitchActions`, `internal/selection/resilience.go`, with `changeModelName`) that sets `state.Provider` from the selected binding/target — after rules run the final handler (`cmd/gateway/handler.go`) reads the post-rule `state.Provider` and passes it to `selection.ResolveTarget`, re-resolving the upstream endpoint on the new provider (the model-keyed redirect pattern; e2e `test/e2e/providers/changeprovider_redirect_test.go`). **A rule-authored `changeProvider` takes precedence over the binding** (GitHub #294): `applyChangeProvider` raises `state.ProviderOverridden`, and the resilience orchestrator (`applyRuleOverride`, `internal/middleware/resilience/middleware.go`) then bypasses the binding-derived policy — group or single-target alias — wholesale, collapsing to one attempt on the rule's provider via `selection.RuleOverrideResilienceConfig` (no provider-switch action, so `buildAttemptState` never clobbers it), logging the bypass and bumping `gateway.resilience.outcome.total{outcome=rule_override}` against the bypassed policy's name; the rule's provider then flows to `selection.ResolveTarget` as usual (e2e `test/e2e/rules/changeprovider_precedence_test.go`). A provider the configuration holds no credential for still fails there (500). `changeModelName` alone leaves the flag clear and runs the binding's policy as normal. `changeApiKey` remains authorable and honoured at the single credential mint site (`resolveCredentialHeaders`, `destination.go`). Two authorable actions write state the v2 data plane never honours: `changeUrl` (`state.UpstreamURL` is written and cloned but never read — `applyStateOverlays`, `cmd/gateway/pipeline.go:313`, touches only `QueryAdditions` and `OutgoingHeaders`) and `useResiliencePolicy` (`state.PolicyRef` is superseded by the binding-derived `ResilienceConfig` on context; `PolicyLookup` is wired to `nil` at `cmd/gateway/handler.go:51`). This is what makes the model-keyed redirect pattern work (a request arriving on `/v1/chat/completions` whose post-rule provider is anthropic lands on anthropic's `chat_completions` endpoint with anthropic's credential + auth header). Don't reintroduce a v1-style endpoint map or override table in the destination builder — keep transport resolution in `selection`, on post-rule state.
