...
- **Config edits via the admin write API apply live** — rules, providers, groups, configurations, api-keys, and connectors all have CRUD endpoints under `/admin/api/v1/config/...` that clone the snapshot, validate, persist back to each block's source file (`config.WriteConfig` routes to the block's `SourceFiles` origin, not a fixed `policy.yaml`), then publish through `config.Store.Replace` — no restart. The data plane reads each request's snapshot atomically, so mid-flight requests see either the pre-swap or post-swap config, never a torn mix. Connector writes also reconcile the runtime sinks: a `config.Store` subscriber (`cmd/gateway/sinks.go`) registers a spool track / webhook pusher for a created connector, stops it for a deleted one, and swaps it (unregister-then-register, same name, backlog kept on disk) for an edited one; a sink that fails to build is logged and its records are counted (`gateway.spool.dropped.total{reason=no_track}`, `gateway.telemetry.push.dropped.total{reason=no_sink}`), never silently dropped. Direct YAML file edits on disk and the `admin`, `telemetry`, `pricing` and `advisors` blocks (none of which have write endpoints) still require a process restart. Automatic `fsnotify`-based reload for that path is still unimplemented (no fsnotify dependency exists) — it remains a backlog item, not a shipped v1.2 feature.
