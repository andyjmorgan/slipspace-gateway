...
func TestValidate_Scanner(t *testing.T) {
	...
	cases := []struct {
		...
		{"finding_suppress empty category", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{OffendingText: "x", Reason: "r"}}}, "finding_suppress[0].category"},
		{"finding_suppress bad category glob", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{Category: "a[b", OffendingText: "x", Reason: "r"}}}, "finding_suppress[0].category"},
		{"finding_suppress missing offending_text", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{Category: "pii.person", Reason: "r"}}}, "offending_text is required"},
		{"finding_suppress bad regex", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{Category: "pii.person", OffendingText: "(", Reason: "r"}}}, "invalid offending_text regex"},
		{"finding_suppress missing reason", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{Category: "pii.person", OffendingText: "x"}}}, "reason is required"},
		{"valid finding_suppress", Scanner{Enabled: true, EvidenceKey: validKey(), Detectors: []Detector{det}, FindingSuppress: []FindingSuppressionRule{{Category: "pii.person", OffendingText: "(?i)^claude code$", Reason: "first-party CC"}}}, ""},
