...
// request path never waits on it, and a wedged or slow Arbiter can
...
type Options struct {
	...
	GatewayID string
	// Secret is the shared HMAC secret the Arbiter verifies against.
	Secret string
...
// Pusher ships records to the Arbiter from a bounded worker pool.
type Pusher struct {
	endpoint  string
	gatewayID string
	secret    []byte
