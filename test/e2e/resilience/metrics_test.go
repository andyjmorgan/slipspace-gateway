...
// per-attempt → per-request meter refactor for gateway_requests_total:
...
func TestMetrics_RequestsTotalIncrementsOncePerRequest(t *testing.T) {
	...
	baseline := parseCounter(baseBody, `gateway_requests_total\{[^}]*model="gpt-4o-mini"[^}]*status_code="200"`)
	...
	for time.Now().Before(deadline) {
		...
		got = parseCounter(body, `gateway_requests_total\{[^}]*model="gpt-4o-mini"[^}]*status_code="200"`)
		if got > baseline {
			break
		}
		time.Sleep(100 * time.Millisecond)
	...
	if got-baseline != 1 {
		t.Errorf("gateway_requests_total{status_code=200,model=gpt-4o-mini} delta = %d; want 1 (one increment per request, not per attempt)", got-baseline)
	}
}
