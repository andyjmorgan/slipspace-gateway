...
bindings pick the backend from the model name. `test_gemini_generate.py` hits
the bare `/v1beta/models/{model}:generateContent` route, parametrized over the
two inbound auth header forms — `bearer` (`Authorization: Bearer sk_live_...`)
and `native-x-goog-api-key` (`x-goog-api-key: sk_live_...`) — for 2 cases per
smoke run, while the OpenAI-compat cases (`test_anthropic_chat.py`,
`test_gemini_chat.py`) point the OpenAI SDK at the unprefixed `{base_url}/v1`
and let the model-keyed bindings land them on the anthropic / gemini
