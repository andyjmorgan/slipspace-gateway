...
type Usage struct {
	...
	// Iterations breaks usage down per server-side agent-loop iteration. Set
	// only on beta server tool-use loop responses (when the relevant beta
	// header is present); nil otherwise. See IterationUsage.
...
// IterationUsage is one entry in Usage.Iterations: the token accounting for a
// single server-side agent-loop iteration. Beta-only — emitted on server
// tool-use loop responses when the relevant beta header is set.
//
// The wire shape is a discriminated union on "type" (sampling "message",
// "compaction", "advisor_message", "fallback_message"), but every current
// variant carries an identical field set, so we model them with one flat
// struct keyed by Type rather than a concrete-type hierarchy. Any new variant
// kind or variant-specific field round-trips via the embedded
// DynamicProperties (invariant #1), so this stays forward-compatible without an
// UnknownX fallback.
// Ref: anthropic-sdk-python beta_iterations_usage.py (BetaIterationsUsageItem).
type IterationUsage struct {
	// Type is the wire "type" discriminator naming the iteration kind:
	// "message", "compaction", "advisor_message", or "fallback_message".
	Type string `json:"type,omitempty"`

	// Model is the model that served this iteration. Empty on iteration
	// kinds that carry no model (e.g. "compaction").
	Model string `json:"model,omitempty"`

	// InputTokens counts input tokens used by this iteration.
	InputTokens int `json:"input_tokens"`

	// OutputTokens counts output tokens generated by this iteration.
	OutputTokens int `json:"output_tokens"`

	// CacheCreationInputTokens counts input tokens written into the prompt
	// cache on this iteration. Nil when not reported.
	CacheCreationInputTokens *int `json:"cache_creation_input_tokens,omitempty"`

	// CacheReadInputTokens counts input tokens served from the prompt cache
	// on this iteration. Nil when not reported.
	CacheReadInputTokens *int `json:"cache_read_input_tokens,omitempty"`

	// CacheCreation breaks this iteration's prompt-cache writes down by TTL
	// tier. Nil when not reported.
	CacheCreation *CacheCreation `json:"cache_creation,omitempty"`
...
// struct into DynamicProperties.Extra.
func (i *IterationUsage) UnmarshalJSON(data []byte) error { return models.UnmarshalDynamic(data, i) }

// MarshalJSON encodes i and merges DynamicProperties.Extra back into the
// resulting object.
func (i IterationUsage) MarshalJSON() ([]byte, error) { return models.MarshalDynamic(i) }
